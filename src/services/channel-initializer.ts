...
export class ChannelInitializer {
  ...
  public async initializeChannels(guild: Guild): Promise<void> {
    try {
      ...
      const isEnabled = await this.configService.getBoolean("voice_channel.enabled") ||
      ...
      const categoryName = await this.configService.getString("voice_channel.category_name") ||
                          await this.configService.getString("VC_CATEGORY_NAME", "Dynamic Voice Channels");

      const lobbyChannelName = await this.configService.getString("voice_channel.lobby_channel_name") ||
      ...
      const announcementChannelName = await this.configService.getString("tracking.weekly_announcement_channel") ||
                                    await this.configService.getString("VC_ANNOUNCEMENT_CHANNEL", "voice-stats");
