...
import { VoiceChannelTracking, IVoiceChannelTracking } from '../models/voice-channel-tracking';

const logger = Logger.getInstance();

export class VoiceChannelTracker {
  private static instance: VoiceChannelTracker;
  private activeSessions: Map<string, { startTime: Date; channelId: string; channelName: string }> = new Map();
  ...
  public async handleVoiceStateUpdate(oldState: VoiceState, newState: VoiceState) {
    try {
      ...
      if (process.env.DEBUG === 'true') {
        logger.info(`[DEBUG] Voice state update for ${member.displayName} (${member.id}):`);
        logger.info(`[DEBUG] Old channel: ${oldChannel ? oldChannel.name : 'none'} (${oldChannel?.id || 'none'})`);
        logger.info(`[DEBUG] New channel: ${newChannel ? newChannel.name : 'none'} (${newChannel?.id || 'none'})`);
        logger.info(`[DEBUG] Active session exists: ${this.activeSessions.has(member.id)}`);
      }

      // User joined a channel (including initial join)
      if (!oldChannel && newChannel) {
        if (process.env.DEBUG === 'true') {
          logger.info(`[DEBUG] Starting tracking for user ${member.displayName} (${member.id}) in channel ${newChannel.name}`);
        }
      ...
      // User switched channels
        ...
        if (process.env.DEBUG === 'true') {
          logger.info(`[DEBUG] Ending tracking for user ${member.displayName} (${member.id}) in old channel ${oldChannel.name}`);
        }
        await this.endTracking(member.id);
        if (process.env.DEBUG === 'true') {
          logger.info(`[DEBUG] Starting tracking for user ${member.displayName} (${member.id}) in new channel ${newChannel.name}`);
        }
        await this.startTracking(member, newChannel.id, newChannel.name);
      }
      ...
      else if ((oldChannel && !newChannel) || (!oldChannel && !newChannel && this.activeSessions.has(member.id))) {
        if (process.env.DEBUG === 'true') {
          logger.info(`[DEBUG] User disconnected - Ending tracking for user ${member.displayName} (${member.id})`);
          const activeSession = this.activeSessions.get(member.id);
          if (activeSession) {
            logger.info(`[DEBUG] Found active session in channel ${activeSession.channelName} (${activeSession.channelId})`);
          }
        }
        await this.endTracking(member.id);
      }
  ...
  private async startTracking(member: GuildMember, channelId: string, channelName: string) {
    try {
  ...
  private async endTracking(userId: string) {
    try {
      ...
      if (!session) {
        if (process.env.DEBUG === 'true') {
          logger.info(`[DEBUG] No active session found for user ${userId} when attempting to end tracking`);
        }
        return;
      }
      ...
      if (process.env.DEBUG === 'true') {
        logger.info(`[DEBUG] Ending session for user ${userId} in channel ${session.channelName} (${session.channelId})`);
      ...
      if (process.env.DEBUG === 'true') {
        if (result) {
          logger.info(`[DEBUG] Successfully updated session for user ${userId}`);
        } else {
          logger.info(`[DEBUG] Failed to find and update session for user ${userId}`);
        }
      }
  ...
  public async getUserStats(userId: string): Promise<IVoiceChannelTracking | null> {
    try {
  ...
  public async getTopUsers(limit: number = 10): Promise<IVoiceChannelTracking[]> {
    try {
