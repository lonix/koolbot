...
export async function execute(interaction: CommandInteraction): Promise<void> {
  try {
    ...
    const response = await axios.get<MarketOrder[]>(
    ...
    const jitaOrders = orders.filter(
      (order) =>
    ...
    await interaction.reply(
      `Current PLEX price in Jita: ${formattedPrice} M ISK`,
    );
    ...
    await interaction.reply({
      content: "An error occurred while fetching PLEX prices.",
      ephemeral: true,
    });
  }
}
