...
export async function execute(interaction: ChatInputCommandInteraction) {
  try {
    ...
    try {
      ...
      logger.warn("Primary EVE Market API failed, trying alternative:", primaryError);
      ...
      try {
        ...
        const buyOrders = orders.filter((order: any) => order.is_buy_order === false).sort((a: any, b: any) => a.price - b.price);
        const sellOrders = orders.filter((order: any) => order.is_buy_order === true).sort((a: any, b: any) => b.price - a.price);
