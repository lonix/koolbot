  ...
  try {
    switch (subcommand) {
      case "add": {
        ...
        const addRoles = (await configService.get<string>("quotes.add_roles"))
          .split(",")
          .filter(Boolean);
        ...
        if (!hasPermission) {
          await interaction.reply({
            content: "You do not have permission to add quotes",
            ephemeral: true,
          });
          return;
        }
      ...
      case "search": {
        ...
        if (quotes.length === 0) {
          await interaction.reply({
            content: "No quotes found matching your search",
            ephemeral: true,
          });
          return;
        }
